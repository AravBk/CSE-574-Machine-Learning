...
def blrObjFunction(initialWeights, *args):
    ...
    nRow = n_features + 1
    col = 1
    wt = initialWeights.reshape(nRow,col)
    
    theta = sigmoid(np.dot(X,wt))
    ...
    error = (- 1.0 * np.sum(error_p) ) / n_data
...
def blrPredict(W, data):
    ...
    lab = np.argmax(sig,axis=1)
    
    label = lab.reshape((N,1)) 
...
def mlrObjFunction(params, *args):
    ...
    top = np.exp(np.dot(X,W))
        
    below = np.sum(top,axis=1)
    below = below.reshape(below.shape[0],1)
    
    theta = top/below
   
    iSum = np.sum(Y*np.log(theta))
...
def mlrPredict(W, data):
    ...
    # Adding biases
    bias = np.full((row,1),1)
    X      = np.concatenate((bias,data), axis=1)

    k = np.sum(np.exp(np.dot(X,W)),axis=1)    
    k = k.reshape(k.shape[0],1)
    
    theta = np.exp(np.dot(X,W))/k
    
    l = np.argmax(theta,axis=1)
    label = l.reshape(row,1)
...
# Linear
linearmod = svm.SVC(kernel='linear')
linearmod.fit(tr_data, tr_label)

print('\n---------Linear Kernel---------\n')
print('\n Training Accuracy -->' + str(100 * linearmod.score(train_data, train_label)) + '%')
print('\n Validation Accuracy -->' + str(100 * linearmod.score(validation_data, validation_label)) + '%')
print('\n Testing Accuracy -->' + str(100 * linearmod.score(test_data, test_label)) + '%')

# RBF with Gamma = 1

rbf_mod = svm.SVC(kernel='rbf', gamma = 1.0)
rbf_mod.fit(tr_data, tr_label)

print('\n---------RBF Kernel with Gamma = 1---------\n')
print('\n Training Accuracy -->' + str(100 * rbf_mod.score(tr_data, tr_label)) + '%')
...
for c in C_values:
    ...
    rbf_mod2.fit(tr_data, tr_label.ravel())
    if inpt <= 10: 
...
rbf_model_comp = svm.SVC(kernel = 'rbf', gamma = 'auto', C = 10)
rbf_model_comp.fit(train_data, train_label.ravel())

print('----------\n RBF with FULL training set with best C : \n------------')
print('\n Training Accuracy:' + str(100 * rbf_model_comp.score(train_data, train_label)) + '%')
print('\n Validation Accuracy:' + str(100 * rbf_model_comp.score(validation_data, validation_label)) + '%')
print('\n Testing Accuracy:' + str(100 * rbf_model_comp.score(test_data, test_label)) + '%')
